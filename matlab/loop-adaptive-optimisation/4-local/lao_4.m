function [solution, J_global, gs_energy] = lao_4(num_spins, num_loops, num_steps, adj, hardness_params, beta_transition)
...
%    addpath('../MonteCarlo/')
    ...
    [J_global, gs_energy] = planted_hamiltonian_4(solution, loops);
    ...
    %   Calculate hardness
    old_hardness = Hardness(hParams, gs_energy, epsilon, beta_h, timeOut, num_runs, 'Met', num_spins);
    ...
    while continue_looping
        ...
        for i = 1:num_replace
            % Generate new loop
            new_loop = random_walk_loop_4( adj );
            % Replace random loop from new loops array with new loop
            new_loops{randi(num_loops)} = new_loop;
        end

        % Calculate planted couplings and energies
        [new_J_global, new_gs_energy] = planted_hamiltonian_4(solution, new_loops);        
        
        % Calculate new Ising problem hardness
        new_hParams = {0, 0, 0, new_J_global, 0};
        new_hardness = Hardness(new_hParams, new_gs_energy, epsilon, beta_h, timeOut, num_runs, 'Met', num_spins);
...
%                 fileNameString = [timeString, '_numqubits_', ...
