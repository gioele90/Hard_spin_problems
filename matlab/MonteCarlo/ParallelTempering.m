...
% Monte Carlo sweeps
    ...
    for i = 1:(num_betas - 1)
        ...
        delta = (betas(i) - betas(i+1)) * -energyChange(newspins, indices_to_flip, 1, 1, 1, h, Jzz, Jzzz,Jzzzz);
        
        %delta = (betas(i) - betas(i+1)) * ...
        %    (Conf_energy(spins_cell{i}, Hparams) - Conf_energy(spins_cell{i+1}, Hparams));

        if rand < exp(delta)
            [spins_cell{i}, spins_cell{i+1}] = deal(spins_cell{i+1}, spins_cell{i});   %Swap two system copies
        end
