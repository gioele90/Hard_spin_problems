function [ out ] = random_walk_loop_3_gpu( adj )
...
%         node_perm = gpuArray.randperm(num_nodes);
%         adj_x = node_perm(1);
        adj_x=gpuArray.randi(num_nodes);
        [adj_y,adj_z] = find(squeeze(adj(adj_x,:,:)));
        rand_ind = gpuArray.randi(length(adj_y));
        ...
        while(~loop_found)            
            ...
            chosen_dims = gpuArray.randperm(3);
            ...
            next_tri = adj_tris(gpuArray.randi(length(adj_tris(:,1))),:);
            % Set previous tri
            prev_tri = loop_seq{end};
            % Add to loop sequence
            loop_seq{end+1} = sort(next_tri);
